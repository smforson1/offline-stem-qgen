// Owner: S3 | Purpose: Connection settings — backend URL and online AI key
...
import { Link2, Zap, Wifi } from 'lucide-react-native';
...
export const SettingsScreen: React.FC = () => {
  ...
  const [localGroqKey, setLocalGroqKey] = useState(settings.geminiApiKey ?? '');

  const save = () => {
    let url = localUrl.trim();
    if (url && !url.startsWith('http://') && !url.startsWith('https://')) url = `http://${url}`;
    settings.setApiUrl(url);
    settings.setGeminiApiKey(localGroqKey.trim());
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeaderRow}>
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              <Text style={styles.cardTitle}>Groq AI Key (Online — Optional)</Text>
              <Text style={styles.cardSubtitle}>When internet is available, uses Groq for fast generation (~3-5 sec). Leave blank to always use local AI.</Text>
            </View>
          </View>
          <TextInput
            value={localGroqKey}
          ...
          />
          <Text style={[styles.cardSubtitle, { marginTop: 8 }]}>
            Free key at console.groq.com — 30 requests/min
          </Text>
        </View>
        ...
        <View style={[styles.card, { backgroundColor: Colors.primarySoft, borderColor: Colors.primary + '33' }]}>
          ...
          <Text style={[styles.cardSubtitle, { lineHeight: 18 }]}>
            • <Text style={{ fontFamily: Fonts.semiBold }}>Offline:</Text> Phone connects to laptop over Wi-Fi hotspot. Both must be on the same network.{'\n'}
            • <Text style={{ fontFamily: Fonts.semiBold }}>Online:</Text> App uses Groq directly from the phone — no laptop needed for question generation.
          </Text>
        </View>
